...
function segmentsFromMetricRows(rows, { metric, labelKey = "label", idKey = "id", colors = {} }) {
  ...
  return rows.map((r) => {
    ...
    const tip =
      metric === "intensity"
        ? `${r[labelKey] ?? id}: ${r.intensity != null ? intFmt.format(Math.round(r.intensity)) : "—"} Eintritte/Film`
...
function metricLabel(metric) {
  ...
  if (metric === "intensity") return "Intensität (Eintritte/Film)";
  return "Nachfrage (Eintritte)";
}
...
function renderKpis(pxRow) {
  ...
  if (!pxRow) return;
  const cards = [
    { k: "Eintritte (Markt)", v: intFmt.format(pxRow.market.demand) },
    { k: "Filme im Programm", v: intFmt.format(pxRow.market.supply) },
    { k: "CH-Anteil (Eintritte)", v: pctFmt.format(pxRow.switzerland.share_demand) },
    ...
    (px.top_countries ?? []).map((c) => ({
      ...
      tip:
        metric === "intensity" && c.intensity
          ? `${c.label}: ${intFmt.format(Math.round(c.intensity))} Eintritte/Film`
          : undefined,
    })),
...
function renderPxTrends(px, metric) {
  ...
  drawMultiLine(
    ...
    [
      { color: "#0b0d10", label: "Eintritte", points: s.market_demand ?? s.market_admissions ?? [] },
  ...
  renderLegend(document.getElementById("marketTrendLegend"), [
    { color: "#0b0d10", label: "Kinoeintritte (Markt)" },
